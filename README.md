...
* Можно было бы обойтись без оркестратора, реализовав распределенные транзакции через хореографию. Однако в этом случае трудно спроектировать содержимое сообщений, потому, что отсутствует точка
